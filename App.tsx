...
const SAMPLE_USERS: User[] = [
  // Teachers
  // { id: 't1', name: 'Dr. Alan Grant', role: 'Professor', department: 'Paleontology', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Alan', registeredAt: new Date().toISOString() },
  // { id: 't2', name: 'Prof. Minerva McGonagall', role: 'Head of Dept', department: 'Transfiguration', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Minerva', registeredAt: new Date().toISOString() },
  // { id: 't3', name: 'Dr. Sheldon Cooper', role: 'Professor', department: 'Physics', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Sheldon', registeredAt: new Date().toISOString() },
  // { id: 't4', name: 'Walter White', role: 'Teacher', department: 'Chemistry', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Walter', registeredAt: new Date().toISOString() },
  // { id: 't5', name: 'Sherlock Holmes', role: 'Lecturer', department: 'Criminology', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Sherlock', registeredAt: new Date().toISOString() },
  // // Students
  // { id: 's1', name: 'Harry Potter', role: 'Student', department: 'General Studies', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Harry', registeredAt: new Date().toISOString() },
  // { id: 's2', name: 'Hermione Granger', role: 'Student', department: 'Computer Science', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Hermione', registeredAt: new Date().toISOString() },
  // { id: 's3', name: 'Ron Weasley', role: 'Student', department: 'Sports Science', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Ron', registeredAt: new Date().toISOString() },
  // { id: 's4', name: 'Peter Parker', role: 'Student', department: 'Photography', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Peter', registeredAt: new Date().toISOString() },
  // { id: 's5', name: 'Miles Morales', role: 'Student', department: 'Physics', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Miles', registeredAt: new Date().toISOString() },
];
...
const Badge: React.FC<{ status: 'Present' | 'Late' | 'Absent' }> = ({ status }) => {
  const colors = {
...
export default function App() {
  ...
  // Load Sample Data on Mount (No LocalStorage)
  useEffect(() => {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!capturedImage) {
          setMessage({ type: 'error', text: 'Could not access camera. Please upload a photo.' });
        }
      }
    }
  };
  ...
  useEffect(() => {
    if ((view === AppView.REGISTER || view === AppView.ATTENDANCE) && !capturedImage) {
      startCamera();
    } else {
      stopCamera();
    }
  ...
  const handleFocusChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (track) {
      try {
        await (track as any).applyConstraints({ advanced: [{ focusMode: 'manual', focusDistance: val }] });
      } catch (err) { console.error(err); }
    }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      if (users.length > 0) {
        ...
        // STRICT Check: Only block if confidence is very high
        if (result.matches && result.matches.length > 0) {
          const match = result.matches[0];
          if (match.confidence > 0.85) {
             const matchUser = users.find(u => u.id === match.userId);
             if (matchUser) {
                setMessage({ type: 'error', text: `Face matches existing user: ${matchUser.name} (${(match.confidence * 100).toFixed(0)}% match).` });
                setLoading(false);
                return;
             }
          }
      ...
      setRegDept('');
      setCapturedImage(null);
      setMessage({ type: 'success', text: `Registered ${newUser.name}. Don't forget to Backup Database!` });
    ...
    try {
      ...
      if (result.matches && result.matches.length > 0) {
        ...
        validMatches.forEach(match => {
          ...
          if (user) {
             const alreadyCheckedIn = records.some(r => 
                r.userId === user.id && new Date(r.timestamp).toDateString() === todayStr
              );
              if (!alreadyCheckedIn) {
                 newRecordsList.push({
                   id: crypto.randomUUID(),
        ...
        if (newCount > 0) {
          ...
          setMessage({ type: 'success', text: `Already present: ${alreadyCount} student(s).` });
        } else {
          setMessage({ type: 'error', text: 'Faces detected but low confidence match.' });
        }
      } else {
  ...
  const updateAttendanceStatus = (userId: string, newStatus: string) => {
    ...
    if (newStatus === 'Absent') {
      ...
      if (existingRecord) {
        ...
        // Set time to 9:00 AM of selected date for consistency, or current time if today
  ...
  const backupRecords = () => {
    if (records.length === 0) {
      setMessage({ type: 'error', text: 'No attendance records to backup.' });
      return;
    }
  ...
  const exportAttendance = () => {
    ...
    records.forEach(r => {
      ...
      const row = [`"${r.userName}"`, r.role, `"${dept}"`, d.toLocaleDateString(), d.toLocaleTimeString(), r.status];
      csvRows.push(row.join(","));
    });
  ...
  const handleImportDatabase = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
        ...
        try {
            ...
            if (!firstLine.includes('image')) {
                setMessage({ type: 'error', text: "Invalid Backup File. Missing image data." });
                return;
            }
            ...
            setUsers(newUsers);
            setMessage({ type: 'success', text: `Database restored! Loaded ${newUsers.length} users.` });
        } catch (err) {
            console.error(err);
            setMessage({ type: 'error', text: "Failed to parse database file." });
        }
    };
  ...
  const handleImportRecords = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
        ...
        try {
            ...
            for (let i = 1; i < rows.length; i++) {
                if (!rows[i].trim()) continue;
                const cols = rows[i].split(','); // Simple split for records as we control the format
                if (cols.length >= 7) {
                    // Handle quoted name if present
                    let name = cols[2];
            ...
            setMessage({ type: 'success', text: `History restored! Loaded ${newRecords.length} records.` });
        } catch (err) {
             console.error(err);
             setMessage({ type: 'error', text: "Failed to parse records file." });
        }
    };
    reader.readAsText(file);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="h-full">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setView(AppView.HISTORY)} className="text-xs text-blue-400 hover:text-blue-300">View All</button>
          </div>
          <div className="space-y-4">
            {records.slice(0, 5).map(record => (
              <div key={record.id} className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-slate-500 mt-1">{new Date(record.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                </div>
              </div>
            ))}
            {records.length === 0 && <p className="text-slate-500 text-center py-4">No recent activity</p>}
          </div>
        </Card>
        ...
        <Card className="h-full">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setView(AppView.REGISTER)} className="text-xs text-blue-400 hover:text-blue-300">Add New</button>
          </div>
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
             {users.map(user => (
               <div key={user.id} className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg border border-slate-700 hover:border-slate-600">
                 <div className="flex items-center space-x-3">
                 ...
                 <button onClick={() => deleteUser(user.id)} className="text-slate-400 hover:text-red-400 p-2"><Trash2 size={16} /></button>
               </div>
             ))}
             {users.length === 0 && <p className="text-slate-500 text-center py-4">No users registered</p>}
          </div>
        </Card>
      </div>
    </div>
  ...
  const renderCameraView = (isRegistration: boolean) => (
    <div className="flex flex-col items-center justify-center h-full space-y-6 max-w-4xl mx-auto">
        ...
        <div className="relative aspect-video rounded-lg overflow-hidden bg-black">
          ...
          {!capturedImage && (
            <div className="absolute inset-0 border-2 border-white/20 m-12 rounded-lg pointer-events-none flex items-center justify-center">
              <div className={`border-2 border-blue-500/50 rounded-lg animate-pulse ${isRegistration ? 'w-64 h-64 rounded-full' : 'w-[90%] h-[80%]'}`}></div>
              <p className="absolute bottom-4 text-white/50 text-sm shadow-black drop-shadow-md">
                 {isRegistration ? 'Align face within frame' : 'Capture whole class or single student'}
              </p>
            </div>
          )}
          ...
          <div className="flex items-center space-x-4 p-2 bg-slate-800/50 mt-1 rounded-b-lg">
             {zoomCap && (
               <div className="flex items-center space-x-2 flex-1">
                 ...
                 <input type="range" min={zoomCap.min} max={zoomCap.max} step={zoomCap.step} value={zoom} onChange={handleZoomChange} className="w-full h-1 bg-slate-600 rounded-lg cursor-pointer" />
               </div>
             )}
             {focusCap && (
               <div className="flex items-center space-x-2 flex-1">
                 <Focus size={16} className="text-slate-400" />
                 <input type="range" min={focusCap.min} max={focusCap.max} step={focusCap.step} value={focus} onChange={handleFocusChange} className="w-full h-1 bg-slate-600 rounded-lg cursor-pointer" />
               </div>
             )}
          </div>
        )}
      ...
      <div className="w-full max-w-xl space-y-4">
        {message && (
          <div className={`p-4 rounded-lg flex items-center space-x-3 ${message.type === 'success' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
            <AlertCircle size={20} /><span>{message.text}</span>
          </div>
        )}
        ...
        {isRegistration ? (
          <div className="space-y-4">
            ...
            <div className="flex space-x-3">
              {!capturedImage ? (
                <>
                  <Button onClick={() => setCapturedImage(captureFrame())} className="flex-1 py-3" disabled={loading}><Camera className="mr-2" size={20} /> Capture</Button>
                  <Button onClick={() => fileInputRef.current?.click()} variant="secondary" className="flex-1 py-3" disabled={loading}><Upload className="mr-2" size={20} /> Upload</Button>
                </>
              ) : (
                <>
                  <Button onClick={retake} variant="secondary" className="flex-1 py-3" disabled={loading}><RefreshCw className="mr-2" size={20} /> Retake</Button>
                  <Button onClick={handleRegister} className="flex-1 py-3" disabled={!regName || !regRole || !regDept || loading}><UserPlus className="mr-2" size={20} /> Register User</Button>
                </>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
             <div className="flex space-x-3">
              {!capturedImage ? (
                 <>
                   <Button onClick={handleAttendance} className="flex-1 py-4 text-lg bg-emerald-600 hover:bg-emerald-500" disabled={loading}><Camera className="mr-2" /> Live Group Scan</Button>
                   <Button onClick={() => fileInputRef.current?.click()} variant="secondary" className="flex-1 py-4 text-lg" disabled={loading}><ImageIcon className="mr-2" /> Upload Group Photo</Button>
                 </>
              ) : (
                 <>
                   <Button onClick={retake} variant="secondary" className="flex-1 py-4" disabled={loading}><RefreshCw className="mr-2" /> Clear</Button>
                   <Button onClick={handleAttendance} className="flex-[2] py-4 text-lg bg-blue-600 hover:bg-blue-500" disabled={loading}><Users className="mr-2" /> Process Attendance</Button>
                 </>
              )}
            </div>
  ...
  const renderHistory = () => {
    ...
    <div className="space-y-6">
      ...
      <input type="file" ref={dbInputRef} onChange={handleImportDatabase} accept=".csv" className="hidden" />
      <input type="file" ref={recordsInputRef} onChange={handleImportRecords} accept=".csv" className="hidden" />

      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-2xl font-bold text-white">Records & Data</h2>
          <p className="text-sm text-slate-400">Manage user database and attendance logs</p>
        </div>
        
        <div className="flex flex-wrap gap-2 w-full xl:w-auto">
           {/* Database Controls */}
           <div className="flex items-center p-1 bg-slate-800 rounded-lg border border-slate-700">
              <Button onClick={() => dbInputRef.current?.click()} variant="secondary" className="text-xs px-3 h-8 bg-transparent border-0">
                 <FileUp size={14} className="mr-1 text-blue-400" /> Load DB
              </Button>
              <div className="w-px h-4 bg-slate-600 mx-1"></div>
              <Button onClick={backupDatabase} variant="secondary" className="text-xs px-3 h-8 bg-transparent border-0">
           ...
           {/* Attendance Controls */}
           <div className="flex items-center p-1 bg-slate-800 rounded-lg border border-slate-700">
              <Button onClick={() => recordsInputRef.current?.click()} variant="secondary" className="text-xs px-3 h-8 bg-transparent border-0">
                 <FileUp size={14} className="mr-1 text-purple-400" /> Load Recs
              </Button>
              <div className="w-px h-4 bg-slate-600 mx-1"></div>
              <Button onClick={backupRecords} variant="secondary" className="text-xs px-3 h-8 bg-transparent border-0">
           ...
           {/* Exports */}
           <div className="flex items-center p-1 bg-slate-800 rounded-lg border border-slate-700">
               <Button onClick={exportUserList} variant="secondary" className="text-xs px-3 h-8 bg-transparent border-0">
                <Users size={14} className="mr-1 text-emerald-400" /> List
              </Button>
              <div className="w-px h-4 bg-slate-600 mx-1"></div>
              <Button onClick={exportAttendance} variant="secondary" className="text-xs px-3 h-8 bg-transparent border-0">
      ...
      <div className="flex items-center justify-between bg-slate-800/50 p-4 rounded-xl border border-slate-700">
         <div className="flex items-center space-x-4">
            <div className="bg-slate-700 p-2 rounded-lg"><Calendar className="text-blue-400" size={20} /></div>
            <div>
               <label className="block text-xs text-slate-400 mb-1">View Date</label>
               <input 
                  type="date" 
                  value={selectedDate} 
         ...
         <div className="text-right">
            ...
            <p className="text-xl font-bold text-green-400">{dailyReport.filter(u => u.status === 'Present' || u.status === 'Late').length}</p>
         </div>
         <div className="text-right">
            <p className="text-xs text-slate-400">Absent</p>
            <p className="text-xl font-bold text-red-400">{dailyReport.filter(u => u.status === 'Absent').length}</p>
         </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-700">
              {dailyReport.map(user => (
                <tr key={user.id} className="hover:bg-slate-700/20 transition-colors text-slate-300">
                  <td className="p-4 flex items-center space-x-3 text-white font-medium">
                    <img src={user.image} alt="" className="w-8 h-8 rounded-full bg-slate-800 object-cover" />
                    <span>{user.name}</span>
                  </td>
                  <td className="p-4">{user.role}</td>
                  <td className="p-4 font-mono text-xs text-slate-400">{user.time}</td>
                  <td className="p-4">
                    <div className="relative inline-block">
                        <select 
                          value={user.status} 
                          onChange={(e) => updateAttendanceStatus(user.id, e.target.value)}
                          className={`
                            appearance-none pl-3 pr-8 py-1 rounded-md text-xs font-medium border outline-none cursor-pointer
                            ${user.status === 'Present' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 
                              user.status === 'Late' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' : 
                        ...
                        <Edit3 size={12} className="absolute right-2 top-1.5 pointer-events-none opacity-50" />
                    </div>
                  </td>
                </tr>
              ))}
              {dailyReport.length === 0 && (
                <tr><td colSpan={4} className="p-8 text-center text-slate-500">No users registered in database.</td></tr>
              )}
            </tbody>
          </table>
        </div>
    ...
    <div className="min-h-screen bg-slate-900 flex text-slate-100 font-sans selection:bg-blue-500/30">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 ${view === item.id ? 'bg-blue-600/10 text-blue-400 border border-blue-600/20' : 'text-slate-400 hover:bg-slate-700/50 hover:text-white'}`}
            >
              <item.icon size={20} />
      ...
      <main className="flex-1 md:ml-64 p-6 lg:p-8 overflow-y-auto">
        <div className="md:hidden flex items-center justify-between mb-6">
           <div className="flex items-center space-x-2"><GraduationCap className="text-blue-500" /><h1 className="text-xl font-bold">CampusID</h1></div>
        </div>
